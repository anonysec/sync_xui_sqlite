...
# فایل کنترل قابلیت غیرفعال‌سازی منقضی‌شده‌ها
...
def restart_xray_core(debug=False):
    ...
    ابتدا پروسس xray رو پیدا می‌کنه و SIGHUP می‌فرسته،
    در صورت شکست fallback به pkill/systemctl می‌کنه.
    """
    try:
        # روش اول: SIGHUP به پروسس xray برای reload بدون restart کامل
        result = subprocess.run(
            ["pkill", "-SIGHUP", "-x", "xray"],
            capture_output=True, timeout=10
        )
        if result.returncode == 0:
            if debug: print("[INFO] xray core reloaded via SIGHUP")
            return True
    except Exception as e:
        if debug: print(f"[WARN] SIGHUP failed: {e}")
    try:
        # روش دوم: kill و restart از طریق x-ui cli (فقط هسته نه پنل)
        result = subprocess.run(
            ["x-ui", "restart"],
            capture_output=True, timeout=15
        )
        if result.returncode == 0:
            if debug: print("[INFO] xray core restarted via x-ui restart")
            return True
        else:
            if debug: print(f"[WARN] x-ui restart returned: {result.returncode}")
    except Exception as e:
        if debug: print(f"[WARN] x-ui restart failed: {e}")
    try:
        # روش سوم: مستقیم پروسس xray رو پیدا کن و kill کن
        # x-ui خودش هسته رو دوباره راه‌اندازی می‌کنه
        result = subprocess.run(
            ["pkill", "-SIGTERM", "-x", "xray"],
            capture_output=True, timeout=10
        )
        if result.returncode == 0:
            if debug: print("[INFO] xray core killed (will be restarted by x-ui watchdog)")
            time.sleep(2)
            return True
    except Exception as e:
        if debug: print(f"[WARN] pkill xray failed: {e}")
    if debug: print("[ERROR] Failed to restart xray core via all methods")
    return False
def is_expire_disable_enabled():
    """چک می‌کنه آیا قابلیت غیرفعال‌سازی منقضی‌شده‌ها فعاله"""
    return os.path.exists(EXPIRE_DISABLE_FLAG)
def check_and_disable_expired(conn, apply=False, debug=False):
    """
    بررسی همه کاربران فعال در همه اینباندها:
    - اگه ترافیکشون تموم شده → غیرفعال کن + ریستارت xray
    - اگه تاریخشون تموم شده → فقط غیرفعال کن (بدون ریستارت)
    
    این تابع مستقل از منطق سینک کار می‌کنه و همه اینباندها رو
    بررسی می‌کنه، حتی اگه کاربر فقط توی یک اینباند باشه.
    """
    if not is_expire_disable_enabled():
        return
    cur = conn.cursor()
    now_ms = int(time.time() * 1000)
    inbs = load_inbounds(conn)
    ct_map = load_ct_map(conn)
    # جمع‌آوری کاربرانی که باید غیرفعال بشن
    # ساختار: { subId: { "by_traffic": bool, "by_date": bool, "entries": [...] } }
    sub_groups = {}
    for iid, settings in inbs:
        for cl in settings.get("clients", []):
            sub = cl.get("subId") or cl.get("subscription")
            if not sub:
                continue
            email = cl.get("email") or ""
            ct_row = ct_map.get((iid, email))
            # فقط کاربران فعال رو بررسی کن
            current_enable = 1
            if ct_row:
                current_enable = int(0 if ct_row.get("enable") in (0, "0", False) else 1)
            
            if current_enable == 0:
                # کاربر قبلاً غیرفعاله، نیازی به بررسی نیست
                continue
            # بررسی منقضی شدن با ترافیک
            quota_gb = 0
            try:
                q = cl.get("totalGB", 0)
                quota_gb = int(q) if q else 0
            except:
                quota_gb = 0
            used_bytes = used_from_ct(ct_row)
            expired_by_traffic = is_expired_by_traffic(quota_gb, used_bytes)
            # بررسی منقضی شدن با تاریخ
            expiry = 0
            try:
            ...
            if expired_by_traffic:
                sub_groups[sub]["by_traffic"] = True
            if expired_by_date:
                sub_groups[sub]["by_date"] = True
            sub_groups[sub]["entries"].append({
                "iid": iid,
                "email": email,
                "ct_row": ct_row,
                "settings": settings,
            ...
            if debug:
                reason = []
                if expired_by_traffic:
                    reason.append(f"traffic({used_bytes}/{quota_gb*1024*1024*1024}B)")
                if expired_by_date:
                    reason.append(f"date(expiry={expiry})")
                print(f"[EXPIRE] sub={sub} iid={iid} email={email} reason={','.join(reason)}")
    if not sub_groups:
        if debug:
            print("[EXPIRE] No expired active clients found")
        return
    # نمایش خلاصه
    total_entries = sum(len(g["entries"]) for g in sub_groups.values())
    print(f"[EXPIRE] Found {len(sub_groups)} subscription(s) with {total_entries} active-but-expired client(s)")
    if not apply:
        if debug:
            for sub, info in sub_groups.items():
                reasons = []
                if info["by_traffic"]: reasons.append("traffic")
                if info["by_date"]: reasons.append("date")
                print(f"[EXPIRE][DRY-RUN] sub={sub} reasons={reasons} count={len(info['entries'])}")
        return
    # --- اعمال تغییرات ---
    need_xray_restart = False
    settings_cache = {}
    def get_settings_cached(iid):
        if iid not in settings_cache:
            cur.execute("SELECT settings FROM inbounds WHERE id=?", (iid,))
            r = cur.fetchone()
            settings_cache[iid] = jload(r[0] if r else "{}")
        return settings_cache[iid]
    def put_settings_cached(iid, s):
    ...
    for sub, info in sub_groups.items():
        ...
        reason_log = []
        if info["by_traffic"]: reason_log.append("traffic")
        if info["by_date"]: reason_log.append("date")
        for entry in info["entries"]:
            iid = entry["iid"]
            email = entry["email"]
            ct_row = entry["ct_row"]
            # 1. غیرفعال کردن در client_traffics
            if ct_row:
                rid = ct_row["row_id"]
                cur.execute(
                ...
                if debug:
                    print(f"[EXPIRE][APPLY] Disabled client_traffics row id={rid} sub={sub} iid={iid} email={email}")
            else:
                # ردیف وجود نداره، بساز
                cur.execute(
                    "INSERT INTO client_traffics(inbound_id, enable, email, up, down, expiry_time, total, reset) VALUES(?,0,?,0,0,0,0,0)",
                    (iid, email)
                )
                if debug:
                    print(f"[EXPIRE][APPLY] Created disabled client_traffics for sub={sub} iid={iid} email={email}")
            # 2. غیرفعال کردن در settings اینباند
            s = get_settings_cached(iid)
            changed = False
            for c in s.get("clients", []):
                c_sub = c.get("subId") or c.get("subscription") or ""
                c_email = c.get("email") or ""
                if c_sub == sub and c_email == email:
                    if c.get("enable", True) not in (False, 0, "false", "0"):
                        c["enable"] = False
                        changed = True
                    break
            if changed:
                put_settings_cached(iid, s)
                if debug:
                    print(f"[EXPIRE][APPLY] Updated inbound settings enable=false sub={sub} iid={iid} email={email}")
        print(f"[EXPIRE][APPLY] sub={sub} disabled {len(info['entries'])} client(s), reason={reason_log}")
    conn.commit()
    # ریستارت xray فقط برای منقضی‌شده‌های ترافیکی
    if need_xray_restart:
        print("[EXPIRE] Traffic-expired clients detected → restarting xray core...")
        success = restart_xray_core(debug=debug)
        if success:
            print("[EXPIRE] xray core restarted successfully")
        else:
            print("[EXPIRE][ERROR] Failed to restart xray core")
    else:
        if debug:
            print("[EXPIRE] Only date-expired clients → no xray restart needed")
def sync_once(conn, apply=False, debug=False):
    ...
    for sub, items in groups.items():
        ...
        for _, e in with_lc:
            ...
            # بررسی تغییرات up و down - فقط اگه مقدار فعلی کمتر از max باشه
            cur_up = int(e["sig"].get("up") or 0)
            cur_down = int(e["sig"].get("down") or 0)
            
            # اگه reset flag فعال باشه، از مقادیر reference استفاده کن
            # در غیر این صورت فقط اگه کمتر از max باشه، به max تغییر بده
            if group_reset_flag:
                ...
                # فقط اگه مقدار فعلی کمتر از max باشه، تغییر بده
    ...
    if not plans:
        ...
        # حتی اگه تغییر سینکی نباشه، بررسی منقضی‌شده‌ها انجام بشه
        if apply:
            check_and_disable_expired(conn, apply=apply, debug=debug)
        return 0
    # --- APPLY ---
    conn.execute("BEGIN")
    cur=conn.cursor()
    ...
    for p in plans:
        ...
        if need_ct:
            ...
            # اطمینان از اینکه quota_db از مجموع up+down کمتر نباشه
    ...
    # بررسی منقضی‌شده‌ها بعد از سینک معمولی
    if apply:
        check_and_disable_expired(conn, apply=apply, debug=debug)
    return len(plans)
def main():
